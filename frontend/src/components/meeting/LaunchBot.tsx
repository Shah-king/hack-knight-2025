...
export const LaunchBot = ({ onBotLaunched, onError }: LaunchBotProps) => {
  ...
  const handleLaunch = async () => {
    if (!meetingUrl.trim()) {
      setErrorMessage("Please enter a Zoom meeting URL or ID");
    ...
    try {
      const response = await fetch(`${apiUrl}/api/zoom/launch`, {
        method: 'POST',
        headers: {
      ...
      console.log('Bot launched successfully:', data);
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to launch bot';
  ...
  const extractMeetingNumber = (input: string) => {
    ...
    <Card className="p-6 bg-card/50 backdrop-blur-glass border-primary/10">
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Join a Zoom meeting as your personalized AI bot
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="meeting-url">
              Zoom Meeting URL or ID <span className="text-destructive">*</span>
            </Label>
            <Input
              id="meeting-url"
            ...
            {meetingNumber && (
              <p className="text-xs text-muted-foreground">
                Meeting ID detected: {meetingNumber}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              id="password"
              type="password"
              placeholder="Enter if meeting is password protected"
              value={password}
          ...
          {launchStatus === 'success' && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/30 text-primary">
              ...
              <span className="text-sm font-medium">AI Assistant launched successfully!</span>
            </div>
          )}
        ...
        <div className="pt-4 border-t border-border/50">
          <p className="text-xs text-muted-foreground">
            Your AI Assistant will join the meeting as a bot participant, listen to conversations,
            transcribe in real-time, and can respond with your voice when needed.
          </p>
        </div>
      </div>
    </Card>
